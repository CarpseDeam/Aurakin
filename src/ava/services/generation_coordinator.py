...
class GenerationCoordinator(BaseGenerationService):
    ...
    using an "Interface Contract" to ensure consistency.
    """
    ...
    async def coordinate_generation(
        ...
        # --- PHASE 1: ARCHITECT DESIGNS THE INTERFACE CONTRACT ---
        self.log("info", "--- Phase 1: Architect is designing the Interface Contract... ---")
        ...
        for i, contract_item in enumerate(interface_contract):
            ...
            # --- Assemble the dynamic context for the Coder ---
            context_blocks = []
            for other_item in interface_contract:
                ...
                members_str = "\n".join([
                    f"    # {sig}" for sig in other_item['public_members']
                ])
            ...
            # --- Local Generation for Boilerplate Files ---
            file_content = ""
            made_api_call = False
