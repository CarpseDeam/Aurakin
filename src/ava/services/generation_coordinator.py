...
class GenerationCoordinator:
    ...
    def robust_clean_llm_output(self, content: str) -> str:
        ...
        # Find all markdown code blocks (e.g., ```python\n...\n```)
        # The regex is non-greedy (`.*?`) to ensure it doesn't span multiple blocks.
        code_block_regex = re.compile(r'```(?:[a-zA-Z0-9_]*)?\n(.*?)\n```', re.DOTALL)
        matches = code_block_regex.findall(content)

        if matches:
            # If markdown code blocks are found, concatenate their contents.
            # This handles cases where the LLM might output explanations then a code block,
            # or even multiple code blocks. We take all the actual code parts.
            return "\n".join(m.strip() for m in matches)
        else:
            # If no markdown code blocks are found, assume the entire content is raw code.
            # This aligns with the `RAW_CODE_OUTPUT_RULE` where the LLM is instructed
            # to provide *only* the raw code.
            return content
