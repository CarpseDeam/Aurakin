...
class ExecutionService:
    ...
    def __init__(self, event_bus: EventBus, project_manager: "ProjectManager"):
        ...
        # This service is called directly by the WorkflowManager, so it doesn't need to subscribe.
        self.log("info", "ExecutionService Initialized.")
    ...
    async def execute_and_capture(self, command: str) -> Tuple[int, str]:
        ...
        full_command = f'"{venv_python}" {command[len("python "):]}' if command.lower().startswith("python ") else f'"{venv_python}" -m {command}'
        full_output = []
